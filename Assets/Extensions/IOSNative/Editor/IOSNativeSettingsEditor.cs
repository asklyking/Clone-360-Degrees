		...
		if (IOSNativeSettings.Instance.ShowOtherParams) {
			...
			if(GUILayout.Button("Remove IOS Native",  GUILayout.Width(140))) {
				...
				int option = EditorUtility.DisplayDialogComplex(
					"Remove IOS Native",
					"Would you like to save IOS Native settings (recommended if you going to use IOS Native later, or you removing it before installing fresh update)",
					"Yes",
					"Cancel",
					"No");
				...
				switch(option) {
				case 0:
					IOSNativeRemoveTool.RemovePlugin();
					break;
				case 2:
					IOSNativeRemoveTool.RemovePlugin(false);
					break;
				}
			}
			...
			if(GUILayout.Button("Remove OneSigal SKD",  GUILayout.Width(140))) {
				...
				if(remove) {
					IOSNativeRemoveTool.RemoveOneSignal();
				}
			}
			EditorGUILayout.EndHorizontal();
